...
from gui.workQThead.difference_analysis.MarkedProQThread import MarkedProQThread
from util.StringUtil import IsFloatNum
from service.FlowChartService import marked_pro_flowchart, scale_flowchart_png
from util.Logger import logger
from util.Util import judge_correlation


class MarkedProWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
        self.step = 1
    ...
    def initUI(self):
        ...
        indexPath = os.path.join(ResourcePath, "img","flowchart","marked_pro.png")
        ...
        # label81 = QLabel("是否为配对样本",self)
        self.cb = QCheckBox()
        self.cb.setText("是否为配对样本")
        # hLayout8.addWidget(label81)
        self.correlationBtn = QPushButton('配置配对样本信息', self, objectName='ClickBtn')
        self.correlationBtn.setStyleSheet("background: #83c5be;max-width: 120px;")
        self.correlationBtn.clicked.connect(self.correlation_sample)
        self.correlationBtn.setVisible(False)

        self.cb.stateChanged.connect(lambda: self.btnstate(self.cb))
        hLayout8.addWidget(self.cb)
        hLayout8.addWidget(self.correlationBtn)
        ...
        self.label72 = QLabel("6. Gene names列格式：", self)
    ...
    def correlation_sample(self):
        marked_pro_information_path = gol.get_value("marked_pro_information_path")
        marked_pro_ngroup = gol.get_value("marked_pro_ngroup")
        marked_pro_pgroup = gol.get_value("marked_pro_pgroup")
        if marked_pro_information_path and marked_pro_ngroup and marked_pro_pgroup:
            try:
                sample_num_match, ngroup_samples, pgroup_samples =judge_correlation(marked_pro_information_path,marked_pro_ngroup,marked_pro_pgroup)
                if sample_num_match:
                    correlationQDialog = CorrelationQDialog()
                    correlationQDialog.initUI(self.resultEdit,"marked_pro",ngroup_samples, pgroup_samples)
                    if correlationQDialog.exec_() == QDialog.Accepted:
                        pass
                else:
    ...
    def certain(self):
        if_correlation = self.cb.isChecked()
        if if_correlation:
            if gol.get_value("marked_pro_correlation_dict") is None:
                QMessageBox.warning(self, "提示", "请配置好配对样本信息")
                return None
            gol.set_value("marked_pro_if_correlation", if_correlation)
        # 校正
        norm = self.normMethod.currentText()
        norm_index = self.normMethod.currentIndex()
        gol.set_value("marked_pro_norm",norm)
        gol.set_value("marked_pro_norm_index",norm_index)
        # 缺失值过滤
        choice_filter_index = self.choice_filter.currentIndex()
        miss_n = self.cb1.value()
        miss_ratio = self.acb1.value()
        gol.set_value("marked_pro_miss_n", miss_n)
        gol.set_value("marked_pro_miss_ratio", miss_ratio)
        gol.set_value("marked_pro_filter_index", choice_filter_index)
        # 缺失值填补
        choice_fill_index = self.choice_fill.currentIndex()
        choice_fill_text = self.choice_fill.currentText()
        gol.set_value("marked_pro_fill_index", choice_fill_index)
        gol.set_value("marked_pro_fill_text", choice_fill_text)
        # p值 or FDR
        ...
        if not IsFloatNum(pvalue):
            ...
            return None
        gol.set_value("marked_pro_p", pvalue)
        gol.set_value("marked_pro_p_threshold_index", p_threshold_index)

        # FC
        fc = self.fcQComboBox.currentText()
        fc_index = self.fcQComboBox.currentIndex()
        gol.set_value("marked_pro_fc", fc)
        gol.set_value("marked_pro_fc_index", fc_index)
        # 富集分析参数
        if_enrich = self.cb_enrich.isChecked()
        gol.set_value("marked_pro_if_enrich", if_enrich)
        if if_enrich:
        ...
        self.main_thread = MarkedProQThread()
    ...
    def expression_data_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step += 1
            gol.set_value("marked_pro_expression_path", fileName)
    ...
    def information_data_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step +=1
            gol.set_value("marked_pro_information_path", fileName)
    ...
    def choose_group(self):
        ...
        '''
        marked_pro_information_path = gol.get_value("marked_pro_information_path")
        if marked_pro_information_path:
            try:
                markedGroupQDialog = MarkedGroupQDialog()
                markedGroupQDialog.initUI(self.resultEdit,"marked_pro",3)
                if markedGroupQDialog.exec_() == QDialog.Accepted:
                    pass
            except Exception as e:
                logger.warning(e)
    ...
    def save_result(self):
        ...
        if directory is not None and directory != "":
            gol.set_value("marked_pro_result_path", directory)
