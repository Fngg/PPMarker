...
from gui.workQThead.difference_analysis.MarkedPhoQThread import MarkedPhoQThread
from util.StringUtil import IsFloatNum
from service.FlowChartService import marked_pho_flowchart, scale_flowchart_png
from util.Logger import logger


class MarkedPhoWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
        self.step = 1
    ...
    def initUI(self):
        ...
        indexPath = os.path.join(ResourcePath, "img","flowchart","marked_pho.png")
        ...
        label10 = QLabel("4. 差异倍数FC的范围：",self)
        self.fcQComboBox = QComboBox(self, minimumWidth=200)
        self.fcQComboBox.addItems(
            ["FC ≥ 1.5 或 FC ≤ 0.67", "FC ≥ 1.25 或 FC ≤ 0.8", "FC ≥ 2 或 FC ≤ 0.5", "FC ≥ 3 或 FC ≤ 0.33",
             "FC ≥ 4 或 FC ≤ 0.25"])
    ...
    def set_col_name(self):
        ...
        if marked_pho_expression_path:
            try:
                choiceQDialog = ChoiceQDialog()
                choiceQDialog.initUI(self.resultEdit, "marked_pho_gene_name", "基因名称对应的列", marked_pho_expression_path)
                if choiceQDialog.exec_() == QDialog.Accepted:
                    pass
            except Exception as e:
                logger.error(e)
                logger.error(f"参数marked_pho_expression_path：{marked_pho_expression_path}")
    ...
    def certain(self):
        ...
        gol.set_value("marked_pho_norm", norm)
        gol.set_value("marked_pho_norm_index", norm_index)
        # 缺失值过滤
        choice_filter_index = self.choice_filter.currentIndex()
        miss_n = self.cb1.value()
        miss_ratio = self.acb1.value()
        gol.set_value("marked_pho_miss_n", miss_n)
        gol.set_value("marked_pho_miss_ratio", miss_ratio)
        gol.set_value("marked_pho_filter_index", choice_filter_index)
        # 缺失值填补
        choice_fill_index = self.choice_fill.currentIndex()
        choice_fill_text = self.choice_fill.currentText()
        gol.set_value("marked_pho_fill_index", choice_fill_index)
        gol.set_value("marked_pho_fill_text", choice_fill_text)
        # p值 or FDR
        ...
        if not IsFloatNum(pvalue):
            ...
            return None
        gol.set_value("marked_pho_p", pvalue)
        gol.set_value("marked_pho_p_threshold_index", p_threshold_index)
        # FC
        fc = self.fcQComboBox.currentText()
        fc_index = self.fcQComboBox.currentIndex()
        gol.set_value("marked_pho_fc", fc)
        gol.set_value("marked_pho_fc_index", fc_index)
        # 富集分析参数
        if_enrich = self.cb_enrich.isChecked()
        gol.set_value("marked_pho_if_enrich", if_enrich)
        if if_enrich:
            ...
            gol.set_value("marked_pho_org_type", enrichOrgType)
            gol.set_value("marked_pho_gene_type", enrichGeneType)

        # 基因名称列
        if_gene_name = self.geneNameCol.isChecked()
        gol.set_value("marked_pho_if_gene_name", if_gene_name)
        if if_gene_name:
            if not gol.get_value("marked_pho_gene_name"):
                QMessageBox.warning(self, "基因名称列", "请设置基因名称对应的列")
                return
        
        # 更新流程图，没有新建线程
        try:
            flowchart_path = marked_pho_flowchart()
        ...
        self.main_thread = MarkedPhoQThread()
    ...
    def expression_data_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step += 1
            gol.set_value("marked_pho_expression_path", fileName)
    ...
    def pro_data_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step +=1
            gol.set_value("marked_pho_prodata_path", fileName)
    ...
    def information_data_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step +=1
            gol.set_value("marked_pho_information_path", fileName)
    ...
    def choose_group(self):
        ...
        if marked_pho_information_path:
            try:
                markedGroupQDialog = MarkedGroupQDialog()
                markedGroupQDialog.initUI(self.resultEdit,"marked_pho",3)
    ...
    def save_result(self):
        ...
        if directory is not None and directory != "":
            gol.set_value("marked_pho_result_path", directory)
