from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QGraphicsPixmapItem, QGraphicsScene, QGraphicsView, \
    QTextEdit, QLabel, QPushButton, QComboBox, QSpinBox, QLineEdit, QMessageBox, QFileDialog, QDialog, QCheckBox
...
from service.FlowChartService import label_free_pro_flowchart, scale_flowchart_png
from gui.workQThead.difference_analysis.LabelFreeProQThread import LabelFreeProQThread
from PyQt5.QtGui import QDoubleValidator
from util.Logger import logger
from util.Util import judge_correlation


class LabelFreeProWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
        self.step = 1
    ...
    def initUI(self):
        ...
        indexPath = os.path.join(ResourcePath, "img","flowchart","label_free_pro.png")
    ...
    def correlation_sample(self):
        ...
        label_free_pro_ngroup = gol.get_value("label_free_pro_ngroup")
        label_free_pro_pgroup = gol.get_value("label_free_pro_pgroup")
        if label_free_pro_information_path and label_free_pro_ngroup and label_free_pro_pgroup:
            try:
                sample_num_match, ngroup_samples, pgroup_samples =judge_correlation(label_free_pro_information_path,label_free_pro_ngroup,label_free_pro_pgroup)
                if sample_num_match:
                    correlationQDialog = CorrelationQDialog()
                    correlationQDialog.initUI(self.resultEdit,"label_free_pro",ngroup_samples, pgroup_samples)
    ...
    def certain(self):
        ...
        if if_correlation:
            if gol.get_value("label_free_pro_correlation_dict") is None:
                QMessageBox.warning(self, "提示", "请配置好配对样本信息")
                return None
            gol.set_value("label_free_pro_if_correlation",if_correlation)
        # 校正
        norm = self.normMethod.currentText()
        norm_index = self.normMethod.currentIndex()
        gol.set_value("label_free_pro_norm",norm)
        gol.set_value("label_free_pro_norm_index",norm_index)
        # 缺失值过滤
        choice_filter_index = self.choice_filter.currentIndex()
        miss_n = self.cb1.value()
        miss_ratio = self.acb1.value()
        gol.set_value("label_free_pro_miss_n",miss_n)
        gol.set_value("label_free_pro_miss_ratio",miss_ratio)
        gol.set_value("label_free_pro_filter_index",choice_filter_index)
        # 缺失值填补
        choice_fill_index = self.choice_fill.currentIndex()
        choice_fill_text = self.choice_fill.currentText()
        gol.set_value("label_free_pro_fill_index", choice_fill_index)
        gol.set_value("label_free_pro_fill_text", choice_fill_text)
        # p值 or FDR
        ...
        if not IsFloatNum(pvalue):
            ...
            return None
        gol.set_value("label_free_pro_p", pvalue)
        gol.set_value("label_free_pro_p_threshold_index", p_threshold_index)
        # FC
        fc = self.fcQComboBox.currentText()
        fc_index = self.fcQComboBox.currentIndex()
        gol.set_value("label_free_pro_fc",fc)
        gol.set_value("label_free_pro_fc_index",fc_index)
        # 富集分析参数
        if_enrich = self.cb_enrich.isChecked()
        gol.set_value("label_free_pro_if_enrich", if_enrich)
        if if_enrich:
        ...
        self.main_thread = LabelFreeProQThread()
    ...
    def expression_data_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step += 1
            gol.set_value("label_free_pro_expression_path", fileName)
    ...
    def information_data_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step +=1
            gol.set_value("label_free_pro_information_path", fileName)
    ...
    def choose_group(self):
        ...
        '''
        label_free_pro_information_path = gol.get_value("label_free_pro_information_path")
        if label_free_pro_information_path:
            try:
                assessmentQDialog = AssessmentQDialog()
                assessmentQDialog.initUI(self.resultEdit,"label_free_pro",2)
    ...
    def save_result(self):
        ...
        if directory is not None and directory != "":
            gol.set_value("label_free_pro_result_path", directory)
