...
from gui.myqdialogs.MarkedGroupQDialog import MarkedGroupQDialog
from gui.myqdialogs.HintQDialog import HintQDialog
from gui.mywidgets.MyQLable import MyQLabel
from gui.workQThead.UtilThread import CopyFileThread
from gui.workQThead.biomarker_analysis.BiomarkerMarkedQThread import BiomarkerMarkedQThread
from service.FlowChartService import biomarker_marked_flowchart, scale_flowchart_png
from util.Logger import logger
from util.StringUtil import IsFloatNum


class BiomarkerMarkedWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
        self.step = 1
    ...
    def initUI(self):
        layout_marked = QHBoxLayout(self)
        layout_marked.setAlignment(Qt.AlignTop)
        layout_marked.setSpacing(10)
        layout_marked.setContentsMargins(20, 20, 10, 10)

        vLayout1_marked = QVBoxLayout()
        vLayout1_marked.setSpacing(10)
        label1 = QLabel("数据分析流程", self)
        # 加载流程图
        ResourcePath = gol.get_value("ResourcePath")
        indexPath = os.path.join(ResourcePath, "img", "flowchart", "biomarker_marked.png")
        ...
        vLayout1_marked.addWidget(label1)
        vLayout1_marked.addWidget(self.picshow)
        vLayout1_marked.addWidget(label2)
        vLayout1_marked.addWidget(self.resultEdit)
        ...
        layout_marked.addLayout(vLayout1_marked, stretch=6)
        # layout_marked.addLayout(vLayout2,stretch=4)
        layout_marked.addWidget(self.scrollArea, stretch=4)
    ...
    def set_col_name(self):
        ...
        if biomarker_marked_expression_path:
            try:
                choiceQDialog = ChoiceQDialog()
                choiceQDialog.initUI(self.resultEdit, "biomarker_marked_gene_name", "基因名称对应的列", biomarker_marked_expression_path)
                if choiceQDialog.exec_() == QDialog.Accepted:
                    pass
            except Exception as e:
                logger.error(e)
                logger.error(f"参数biomarker_marked_expression_path：{biomarker_marked_expression_path}")

    def set_new_col_name(self):
        biomarker_marked_test_expression_path = gol.get_value("biomarker_marked_test_data_path")
        if biomarker_marked_test_expression_path:
            try:
                choiceQDialog = ChoiceQDialog()
                choiceQDialog.initUI(self.resultEdit, "biomarker_marked_test_gene_name", "基因名称对应的列", biomarker_marked_test_expression_path)
                if choiceQDialog.exec_() == QDialog.Accepted:
                    pass
            except Exception as e:
                logger.error(e)
                logger.error(f"参数biomarker_marked_test_expression_path：{biomarker_marked_test_expression_path}")
    ...
    def test_information_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step +=1
            gol.set_value("biomarker_marked_test_information_path", fileName)

    def clear_test_data(self):
        fileName = gol.remove_value("biomarker_marked_test_data_path")
        if len(fileName) > 0:
    ...
    def test_data_import(self):
        ...
        if fileName is not None and fileName!="":
            ...
            self.step +=1
            gol.set_value("biomarker_marked_test_data_path", fileName)
    ...
    def hint(self):
        ...
        '''
        try:
            hintQDialog = HintQDialog()
            hintQDialog.initUI("无标记定量的样本信息表", "无标记定量的样本信息表需要标记上样批次，在同一批次中用一个质控组进行校正。")
            if hintQDialog.exec_() == QDialog.Accepted:
                pass
        except Exception as e:
            logger.warning(e)

    def feature_select_change(self,tag):
        if tag in [0,2]:
            self.pvalueEdit.setVisible(True)
            self.fcQComboBox.setVisible(True)
            self.label16.setVisible(True)
            self.label17.setVisible(True)
        else:
            self.pvalueEdit.setVisible(False)
            self.fcQComboBox.setVisible(False)
    ...
    def expression_data_import(self):
        ...
        if fileName is not None and fileName != "":
            ...
            self.step += 1
            gol.set_value("biomarker_marked_expression_path", fileName)
    ...
    def certain(self):
        ...
        gol.set_value("biomarker_marked_miss_n", miss_n)
        gol.set_value("biomarker_marked_miss_ratio", miss_ratio)
        gol.set_value("biomarker_marked_filter_index", choice_filter_index)
        ...
        if not IsFloatNum(pvalue):
            ...
            return None
        gol.set_value("biomarker_marked_p", pvalue)
        gol.set_value("biomarker_marked_p_threshold_index", p_threshold_index)

        fc = self.fcQComboBox.currentText()
        fc_index = self.fcQComboBox.currentIndex()
        gol.set_value("biomarker_marked_fc", fc)
        gol.set_value("biomarker_marked_fc_index", fc_index)
        ...
        gol.set_value("biomarker_marked_norm",norm)
        gol.set_value("biomarker_marked_norm_index",norm_index)
        gol.set_value("biomarker_marked_miss_n",miss_n)
        gol.set_value("biomarker_marked_imputation",imputation)
        gol.set_value("biomarker_marked_imputation_index",imputation_index)
        gol.set_value("biomarker_marked_feature_select",feature_select)
        gol.set_value("biomarker_marked_feature_select_index",feature_select_index)
        gol.set_value("biomarker_marked_max_feature_num",max_feature_num)
        gol.set_value("biomarker_marked_classifier",classifier)
        gol.set_value("biomarker_marked_classifier_index",classifier_index)
        gol.set_value("biomarker_marked_train_ratio",train_ratio)
        gol.set_value("biomarker_marked_test_ratio",test_ratio)
        gol.set_value("biomarker_marked_verification_ratio",verification_ratio)

        # 基因名称列
        if_gene_name = self.geneNameCol.isChecked()
        gol.set_value("biomarker_marked_if_gene_name", if_gene_name)
        if if_gene_name:
            if not gol.get_value("biomarker_marked_gene_name"):
                QMessageBox.warning(self, "基因名称列", "请设置基因名称对应的列")
                return
        test_if_gene_name = self.newGeneNameCol.isChecked()
        gol.set_value("biomarker_marked_test_if_gene_name", test_if_gene_name)
        if test_if_gene_name:
            if not gol.get_value("biomarker_marked_test_gene_name"):
                QMessageBox.warning(self, "基因名称列", "请设置基因名称对应的列")
                return

        # 更新流程图，没有新建线程
        try:
            flowchart_path = biomarker_marked_flowchart()
        ...
        self.main_thread = BiomarkerMarkedQThread()
    ...
    def save_result(self):
        ...
        if directory is not None and directory != "":
            gol.set_value("biomarker_marked_result_path", directory)
    ...
    def choose_group(self):
        ...
        if biomarker_marked_information_path:
            try:
                assessmentQDialog = MarkedGroupQDialog()
                assessmentQDialog.initUI(self.resultEdit, "biomarker_marked", 3)
    ...
    def information_data_import(self):
        ...
        if fileName is not None and fileName != "":
            ...
            self.step += 1
            gol.set_value("biomarker_marked_information_path", fileName)