...
from service.difference_analysis.UtilService import create_result_dir, get_groups_dict_list, \
    export_data, extract_gene_name, check_data_marked_pro
...
class MarkedProQThread(QThread):
    # 失败时传出信号
    ...
    def __int__(self):
        super(MarkedProQThread, self).__init__()
        self.pdf = None
        self.i = 0
    ...
    def _run(self):
        ...
        logger.debug("标记定量蛋白质组学分析开始")
        marked_pro_expression_path = gol.get_value("marked_pro_expression_path")
        marked_pro_information_path = gol.get_value("marked_pro_information_path")
        marked_pro_result_path = gol.get_value("marked_pro_result_path")
        if_enrich = gol.get_value("marked_pro_if_enrich")
        if_gene_name = gol.get_value("marked_pro_if_gene_name")
        if if_gene_name:
            gene_col_name = gol.get_value("marked_pro_gene_name")
        else:
            gene_col_name = "Gene names"
        # pdf记录分析过程
        pdf_path = os.path.join(marked_pro_result_path, "readme.pdf")
        self.pdf = ResultPdf(pdf_path, "标记定量蛋白质组学数据分析结果")

        # 创建结果文件夹
        create_result_dir(marked_pro_result_path,if_enrich)

        expression_df = readFile(marked_pro_expression_path,if_zero_na=True, index_col=None)
        information_df = readFile(marked_pro_information_path,if_zero_na=False, index_col=None)
        # 检测数据是否符合要求
        ...
        # 数据质量评估
        data_assesment(expression_df, marked_pro_result_path, groups_dict,groups,simples)
        self.info_record("数据质量评估", "生成数据分布/数据缺失情况/样本相关性/样本聚类等图")
        # 校正
        # 列校正
        norm_index = gol.get_value("marked_pro_norm_index")
        if norm_index == 0:
            marked_pro_norm = "upper quantile normalization"
            expression_norm_df = upper_quantile_norm(expression_df)
        elif norm_index == 1:
            marked_pro_norm = "quantile normalization"
            expression_norm_df = quantile_norm(expression_df)
        elif norm_index ==3:
            marked_pro_norm = "flatten sum normalization"
            expression_norm_df = flatten_sum_norm(expression_df)
        elif norm_index==4:
            marked_pro_norm = "FOT normalization"
            expression_norm_df = FOT_norm(expression_df)
        else:
            marked_pro_norm = "no normalization"
            expression_norm_df = expression_df
        if marked_pro_norm != "no normalization":
            self.info_record("数据列校正", marked_pro_norm + "-样本间（列）校正")
            data_norm(expression_df, expression_norm_df, marked_pro_norm, marked_pro_result_path,groups_dict)
            export_data(expression_norm_df, marked_pro_result_path, Normalization_and_Filtering_dirname,
                        "alfter_normalization.xlsx", marked_pro_norm)
        # 行校正
        marked_pro_qcgroup = gol.get_value("marked_pro_qcgroup") # 质控组
        expression_ratio_df = ratio(expression_norm_df,information_df,marked_pro_qcgroup)
        # 删除质控组
        qcgroup_names = list(information_df.loc[information_df["group"] == marked_pro_qcgroup, "name"])
        expression_ratio_df.drop(columns=qcgroup_names, inplace=True)
        information_df = information_df[information_df['group']!=marked_pro_qcgroup]
        groups_dict, groups, simples = get_groups_dict_list(information_df)

        self.info_record("数据行校正", "做ratio-蛋白（行）校正")
        data_norm(expression_norm_df, expression_ratio_df, "ratio", marked_pro_result_path,groups_dict,"Show_Normalization_and_ratio.pdf")
        export_data(expression_ratio_df, marked_pro_result_path, Normalization_and_Filtering_dirname,
                    "alfter_normalization_and_ratio.xlsx", "ratio")

        # 数据过滤
        filter_index = gol.get_value("marked_pro_filter_index")
        if filter_index == 0:
            miss_n = gol.get_value("marked_pro_miss_n")
            expression_filter_df = filter_by_n(expression_ratio_df, miss_n, groups)
            filter_rule = f"各组中非缺失值数量低于{miss_n}过滤掉"
        elif filter_index == 1:
            miss_ratio = gol.get_value("marked_pro_miss_ratio")
            if miss_ratio>1:
        ...
        export_data(expression_filter_df, marked_pro_result_path, Normalization_and_Filtering_dirname,
                    "alfter_normalization_and_filter.xlsx", "normalization and filter data")
        data_filter(expression_ratio_df, expression_filter_df, None, marked_pro_result_path, groups_dict)

        # 缺失值填补
        fill_index = gol.get_value("marked_pro_fill_index")
        fill_text = gol.get_value("marked_pro_fill_text")
        if fill_index == 0:
            # 不填补
            marked_imputation = "Not filling"
            expression_imputation_df = expression_filter_df
        elif fill_index == 1:
            # 最小值填补
            marked_imputation = "minimum filling"
            expression_imputation_df = base_missing_filing(expression_filter_df, groups=groups)
        elif fill_index == 2:
            # k近邻填补
            marked_imputation = "KNN filling"
            expression_imputation_df = knn_missing_filter(expression_filter_df)
        else:
            # 随机森林填补
            marked_imputation = "random forest filling"
            expression_imputation_df = forest_missing_filing(expression_filter_df)
        if fill_index != 0:
            self.info_record("缺失值填补", fill_text)
            data_imputation(expression_filter_df, expression_imputation_df, marked_imputation,
                            marked_pro_result_path, groups_dict, Normalization_and_Filtering_dirname)
            export_data(expression_imputation_df, marked_pro_result_path, Normalization_and_Filtering_dirname,
                        "normalization_filter_imputation.xlsx", "normalization_filter_imputation")
        # 差异蛋白分析
        fc_index = gol.get_value("marked_pro_fc_index")
        fc_despri = gol.get_value("marked_pro_fc")
        if fc_index == 0:
            fc = 1.5
        elif fc_index == 1:
            fc = 1.25
        ...
        if_correlation = gol.get_value("marked_pro_if_correlation")
        pgroup = gol.get_value("marked_pro_pgroup") # 阳性组
        pro_p_threshold_index = gol.get_value("marked_pro_p_threshold_index")
        p = gol.get_value("marked_pro_p")  # P值or FDR值
        ...
        if if_correlation is not None and if_correlation==True:
            # 配对样本检验
            correlation_dict = gol.get_value("marked_pro_correlation_dict")
        ...
        export_data(all, marked_pro_result_path, Difference_Analysis_dirname,
                    "difference_analysis.xlsx", "difference analysis result data",
                    complement_expression_df.loc[all.index, :])
        data_dfp_analysis(all,fc,groups,simples,groups_dict,marked_pro_result_path,p_value=float(p),pvalue_colname=pvalue_colname)
        ...
        # heatmap_result_path = os.path.join(marked_pro_result_path, Difference_Analysis_dirname)
        # # result = draw_r_heatmap_service_fuc(tmp[tmp["DEP"]!="none-significant"],groups_dict,simples,heatmap_result_path)
        ...
        if if_enrich:
            if "Gene names_No." == all.index.name:
                enrichOrgType = gol.get_value("marked_pro_org_type")
                enrichGeneType = gol.get_value("marked_pro_gene_type")
                ...
                if len(up_genes) > 0:
                    enrich_result_path_up = os.path.join(marked_pro_result_path, Enrichment_Analysis_dirname,
                                                         "up")
                ...
                if len(down_genes) > 0:
                    enrich_result_path_down = os.path.join(marked_pro_result_path, Enrichment_Analysis_dirname,
                                                           "down")
