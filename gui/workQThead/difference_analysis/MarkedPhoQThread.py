...
from service.difference_analysis.UtilService import create_result_dir, get_groups_dict_list, \
    export_data, extract_gene_name, check_data_marked_pho
...
class MarkedPhoQThread(QThread):
    # 失败时传出信号
    ...
    def __int__(self):
        super(MarkedPhoQThread, self).__init__()
        self.pdf = None
        self.i = 0
    ...
    def _run(self):
        ...
        logger.debug("标记定量磷酸化组学分析开始")
        marked_pho_expression_path = gol.get_value("marked_pho_expression_path")
        ...
        # 创建结果文件夹
        create_result_dir(marked_pho_result_path,if_enrich)

        expression_df = readFile(marked_pho_expression_path,if_zero_na=True, index_col=None)
        information_df = readFile(marked_pho_information_path,if_zero_na=False, index_col=None)
        pro_dfp_df = readFile(marked_pho_prodata_path,if_zero_na=False,sheet_name=0)
        pro_supplementary_df = readFile(marked_pho_prodata_path,if_zero_na=False,sheet_name=1)
        # 检测数据是否符合要求
        result, despri = check_data_marked_pho(expression_df, information_df,pro_dfp_df,pro_supplementary_df)
        if not result:
        ...
        # 数据质量评估
        data_assesment(expression_df, marked_pho_result_path, groups_dict,groups,simples)
        self.info_record("数据质量评估", "生成数据分布/数据缺失情况/样本相关性/样本聚类等图")
        # 校正
        # 列校正
        norm_index = gol.get_value("marked_pho_norm_index")
        if norm_index == 0:
            marked_pho_norm = "upper quantile normalization"
            expression_norm_df = upper_quantile_norm(expression_df)
        elif norm_index == 1:
            marked_pho_norm = "quantile normalization"
            expression_norm_df = quantile_norm(expression_df)
        elif norm_index ==3:
            marked_pho_norm = "flatten sum normalization"
            expression_norm_df = flatten_sum_norm(expression_df)
        elif norm_index==4:
            marked_pho_norm = "FOT normalization"
            expression_norm_df = FOT_norm(expression_df)
        else:
            marked_pho_norm = "no normalization"
            expression_norm_df = expression_df
        if marked_pho_norm != "no normalization":
            self.info_record("数据列校正", marked_pho_norm + "-样本间（列）校正")
            data_norm(expression_df, expression_norm_df, marked_pho_norm, marked_pho_result_path,groups_dict)
            export_data(expression_norm_df, marked_pho_result_path, Normalization_and_Filtering_dirname,
                        "alfter_normalization.xlsx", marked_pho_norm)
        # 行校正
        marked_pho_qcgroup = gol.get_value("marked_pho_qcgroup") # 质控组
        expression_ratio_df = ratio(expression_norm_df,information_df,marked_pho_qcgroup)
        # 删除质控组
        qcgroup_names = list(information_df.loc[information_df["group"] == marked_pho_qcgroup, "name"])
        expression_ratio_df.drop(columns=qcgroup_names, inplace=True)
        information_df = information_df[information_df['group']!=marked_pho_qcgroup]
        groups_dict, groups, simples = get_groups_dict_list(information_df)

        self.info_record("数据行校正", "做ratio-蛋白（行）校正")
        data_norm(expression_norm_df, expression_ratio_df, "ratio", marked_pho_result_path,groups_dict,"Show_Normalization_and_ratio.pdf")
        export_data(expression_ratio_df, marked_pho_result_path, Normalization_and_Filtering_dirname,
                    "alfter_normalization_and_ratio.xlsx", "ratio")

        # 数据过滤
        filter_index = gol.get_value("marked_pho_filter_index")
        if filter_index == 0:
            miss_n = gol.get_value("marked_pho_miss_n")
            expression_filter_df = filter_by_n(expression_ratio_df, miss_n, groups)
            filter_rule = f"各组中非缺失值数量低于{miss_n}过滤掉"
        elif filter_index == 1:
            miss_ratio = gol.get_value("marked_pho_miss_ratio")
            if miss_ratio > 1:
        ...
        export_data(expression_filter_df, marked_pho_result_path, Normalization_and_Filtering_dirname,
                    "alfter_normalization_and_filter.xlsx", "normalization and filter data")
        data_filter(expression_ratio_df, expression_filter_df, None, marked_pho_result_path, groups_dict)

        # 缺失值填补
        fill_index = gol.get_value("marked_pho_fill_index")
        fill_text = gol.get_value("marked_pho_fill_text")
        if fill_index == 0:
            # 不填补
        ...
        if fill_index != 0:
            ...
            data_imputation(expression_filter_df, expression_imputation_df, label_free_imputation,
                            marked_pho_result_path, groups_dict, Normalization_and_Filtering_dirname)
            export_data(expression_imputation_df, marked_pho_result_path, Normalization_and_Filtering_dirname,
                        "normalization_filter_imputation.xlsx", "normalization_filter_imputation")

        # 差异蛋白分析
        fc_index = gol.get_value("marked_pho_fc_index")
        fc_despri = gol.get_value("marked_pho_fc")
        if fc_index == 0:
            fc = 1.5
        elif fc_index == 1:
            fc = 1.25
        ...
        pgroup = gol.get_value("marked_pho_pgroup") # 阳性组
        pro_p_threshold_index = gol.get_value("marked_pho_p_threshold_index")
        p = gol.get_value("marked_pho_p")  # P值or FDR值
        ...
        # 与蛋白组的差异表达结果做校正
        finally_all = pho_dfp_base_pro(all, complement_expression_df.loc[all.index, :], pro_dfp_df,
                                       pro_supplementary_df, fc,p=float(p),pvalue_colname=pvalue_colname)
        ...
        self.info_record("差异分析",
                         "用蛋白质组的差异结果对磷酸化组的差异结果进行校正;"+p_despri + fc_despri + "; 可视化展示：火山图，聚类图，热图。一共得到差异上调基因" + str(
                             up_genes_len) + "个，差异下调基因" + str(down_genes_len) + "个。")
        export_data(finally_all, marked_pho_result_path, Difference_Analysis_dirname,
                    "difference_analysis.xlsx", "difference analysis result data",
                    complement_expression_df.loc[finally_all.index, :])
        data_dfp_analysis(finally_all,fc,groups,simples,groups_dict,marked_pho_result_path,pvalue_colname=pvalue_colname,log2fc_colname ="finally_log2FC",dfp_colname="finally_DEP",p_value=float(p))
        ...
        # heatmap_result_path = os.path.join(marked_pho_result_path, Difference_Analysis_dirname)
        ...
        if if_enrich:
            if "Gene names_No." == finally_all.index.name:
                enrichOrgType = gol.get_value("marked_pho_org_type")
                enrichGeneType = gol.get_value("marked_pho_gene_type")
                ...
                if len(up_genes)>0:
                    enrich_result_path_up = os.path.join(marked_pho_result_path, Enrichment_Analysis_dirname,"up")
                ...
                if len(down_genes)>0:
                    enrich_result_path_down = os.path.join(marked_pho_result_path, Enrichment_Analysis_dirname, "down")
