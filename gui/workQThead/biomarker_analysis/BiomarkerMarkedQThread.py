...
from service.difference_analysis.UtilService import  extract_gene_name, get_groups_dict_list, \
...
class BiomarkerMarkedQThread(QThread):
    # 失败时传出信号
    ...
    def __int__(self):
        super(BiomarkerMarkedQThread, self).__init__()
        self.pdf = None
        self.i = 0
    ...
    def difference_analysis_select(self,data,groups_dict,pgroup,result_path,complement_expression_df,p,if_p):
        # 差异蛋白分析
        fc_index = gol.get_value("biomarker_marked_fc_index")
        if fc_index == 0:
            fc = 1.5
        elif fc_index == 1:
            fc = 1.25
        ...
        analysis = dfp_analysis(data, groups_dict, FC_threshold=fc, E_group=pgroup, p=float(p),if_p=if_p, already_log2=False)
    ...
    def feature_select_fuc(self,X_train,y_train,groups_dict,groups,simples,pgroup,ngroup,result_path,complement_expression_df):
        feature_select_index = gol.get_value("biomarker_marked_feature_select_index")
        feature_select_str = gol.get_value("biomarker_marked_feature_select")
        # fc_despri = gol.get_value("biomarker_marked_fc","")
        max_feature_num = gol.get_value("biomarker_marked_max_feature_num")
        ...
        # P值
        pro_p_threshold_index = gol.get_value("biomarker_marked_p_threshold_index")
        dir_path = os.path.join(result_path, biomarker_feature_select_dirname)
        p = gol.get_value("biomarker_marked_p")  # P值or FDR值
        ...
        if feature_select_index in [0, 1, 2, 3, 4]:
            ...
            difference_analysis_df, analysis, fc, dfp_len = self.difference_analysis_select(difference_analysis_data,
                                                                                            groups_dict, pgroup,
                                                                                            result_path,
                                                                                            complement_expression_df, p,
                                                                                            if_p)

            difference_analysis_plot(difference_analysis_df, fc, groups, simples, groups_dict, pdf, p_value=float(p),
                                     dir_path=dir_path, pvalue_colname=pvalue_colname)
        ...
        if feature_select_index == 0:
            ...
            # 差异基因+Lasso
            feature_select_result = feature_select(X_train_prepared, y_train, list(X_train_prepared.columns),
                                                   max_feature_num, method="lasso")
            ...
            export_data(difference_analysis_model_df, result_path, biomarker_feature_select_dirname,
                        "difference_analysis_Lasso_Feature_Selection.xlsx", "Feature_Selection",
                        complement_expression_df.loc[difference_analysis_model_df.index, :])

            feature_select_plot(feature_select_result, difference_analysis_data, groups_dict, groups, pdf,
                                dir_path=dir_path)
        ...
        draw_pca_cluster(difference_analysis_data.loc[final_select_proteins, simples], groups_dict, "Final Select Proteins", pdf)
        pdf.close()

        PCA_file_path = os.path.join(result_path, biomarker_feature_select_dirname,
                                     "PCA_based_on_Final_Select_Proteins.html")
        pca_plot_html(difference_analysis_data.loc[final_select_proteins, simples], PCA_file_path, groups_dict,
                      title="Final Select Proteins")

        self.info_record("特征筛选", "经过'"+feature_select_str+"'特征筛选，得到的最终特征为："+str(final_select_proteins)+",一共有"+str(len(final_select_proteins))+"个基因。")
    ...
    def _run(self):
        ...
        # 创建结果文件夹
        create_result_dir(biomarker_marked_result_path)

        expression_df = readFile(biomarker_marked_expression_path,if_zero_na=True, index_col=None)
        information_df = readFile(biomarker_marked_information_path,if_zero_na=False, index_col=None)
        # 检测数据是否符合要求
        result, despri = check_data_marked_pro(expression_df, information_df)
        if not result:
        ...
        # 列校正
        norm_index = gol.get_value("biomarker_marked_norm_index")
        if norm_index == 0:
            biomarker_marked_norm = "upper quantile normalization"
            expression_norm_df = upper_quantile_norm(expression_df)
        elif norm_index == 1:
            biomarker_marked_norm = "quantile normalization"
            expression_norm_df = quantile_norm(expression_df)
        elif norm_index ==3:
            biomarker_marked_norm = "flatten sum normalization"
            expression_norm_df = flatten_sum_norm(expression_df)
        elif norm_index==4:
            biomarker_marked_norm = "FOT normalization"
            expression_norm_df = FOT_norm(expression_df)
        else:
            biomarker_marked_norm = "no normalization"
            expression_norm_df = expression_df
        self.info_record("数据校正", biomarker_marked_norm + "-样本间（列）校正")
        data_norm(expression_df, expression_norm_df, biomarker_marked_norm, biomarker_marked_result_path,groups_dict,dirname=biomarker_preprocess_dirname)
        export_data(expression_norm_df, biomarker_marked_result_path, biomarker_preprocess_dirname,
                    "normalization.xlsx", biomarker_marked_norm)
        # 行校正
        biomarker_marked_qcgroup = gol.get_value("biomarker_marked_qcgroup") # 质控组
        expression_ratio_df = ratio(expression_norm_df,information_df,biomarker_marked_qcgroup)
        # 删除质控组
        qcgroup_names = list(information_df.loc[information_df["group"] == biomarker_marked_qcgroup, "name"])
        expression_ratio_df.drop(columns=qcgroup_names, inplace=True)
        information_df = information_df[information_df['group']!=biomarker_marked_qcgroup]
        groups_dict, groups, simples = get_groups_dict_list(information_df)

        self.info_record("数据行校正", "做ratio-蛋白（行）校正")
        data_norm(expression_norm_df, expression_ratio_df, "ratio", biomarker_marked_result_path,groups_dict,file_name="Show_Normalization_and_ratio.pdf",dirname=biomarker_preprocess_dirname)
        export_data(expression_ratio_df, biomarker_marked_result_path, biomarker_preprocess_dirname,
                    "normalization_and_ratio.xlsx", "normalization_ratio")

        # 数据过滤
        filter_index = gol.get_value("biomarker_marked_filter_index")
        if filter_index == 0:
            miss_n = gol.get_value("biomarker_marked_miss_n")
            expression_filter_df = filter_by_n(expression_ratio_df, miss_n, groups)
            filter_rule = f"各组中非缺失值数量低于{miss_n}过滤掉"
        elif filter_index == 1:
            miss_ratio = gol.get_value("biomarker_marked_miss_ratio")
            if miss_ratio > 1:
                miss_ratio = float(miss_ratio / 100)
            expression_filter_df = filter_by_ratio(expression_ratio_df, miss_ratio, groups)
            filter_rule = f"各组中缺失值比例高于{miss_ratio}%过滤掉"
        else:
            raise Exception("The selected missing value filtering rule is out of range")
        filter_row_num = len(expression_ratio_df) - len(expression_filter_df)
        self.info_record("数据过滤", "分组过滤，" + filter_rule + "，一共过滤掉" + str(filter_row_num) + "行。")
        export_data(expression_filter_df, biomarker_marked_result_path, biomarker_preprocess_dirname,
                    "normalization_and_filter.xlsx", "normalization_filter")
        data_filter(expression_ratio_df, expression_filter_df, None, biomarker_marked_result_path, groups_dict,
                    dirname=biomarker_preprocess_dirname)

        # 缺失值填补
        imputation_index = gol.get_value("biomarker_marked_imputation_index")
        if imputation_index == 0:
            biomarker_marked_imputation = "minimum filling"
            expression_imputation_df = base_missing_filing(expression_filter_df,groups=groups)
        elif imputation_index == 1:
            biomarker_marked_imputation = "KNN filling"
            expression_imputation_df = knn_missing_filter(expression_filter_df)
        else:
            biomarker_marked_imputation = "random forest filling"
            expression_imputation_df = forest_missing_filing(expression_filter_df)
        self.info_record("缺失值填补", "填补方式："+biomarker_marked_imputation)
        data_imputation(expression_filter_df, expression_imputation_df, biomarker_marked_imputation, biomarker_marked_result_path, groups_dict,biomarker_preprocess_dirname)
        export_data(expression_imputation_df, biomarker_marked_result_path, biomarker_preprocess_dirname,
        ...
        # TMT 做了ratio不能再做log2
        # 数据集拆分
        train_ratio = gol.get_value("biomarker_marked_train_ratio")
        test_ratio = gol.get_value("biomarker_marked_test_ratio")
        verification_ratio = gol.get_value("biomarker_marked_verification_ratio")
        ...
        tips_result,tips_despri = tips(groups,train_ratio,test_ratio,verification_ratio,"biomarker_marked_test_data_path")
        if not tips_result:
        ...
        ngroup = gol.get_value("biomarker_marked_ngroup") # 阴性
        pgroup = gol.get_value("biomarker_marked_pgroup") # 阳性
        ...
        export_data(train_set.T, biomarker_marked_result_path, biomarker_data_split_dirname,
                    "train_set.xlsx", "train set")
        if test_set is not None:
            export_data(test_set.T, biomarker_marked_result_path, biomarker_data_split_dirname,
                        "test_set.xlsx", "test set")
        if verification_set is not None:
            export_data(verification_set.T, biomarker_marked_result_path, biomarker_data_split_dirname,
        ...
        select_proteins, select_proteins_import_df = self.feature_select_fuc(X_train,y_train,groups_dict,groups,simples,pgroup,ngroup,biomarker_marked_result_path,complement_expression_df)
        if select_proteins is None or len(select_proteins)==0:
            self.over(start_time, False)
            return None
        final_select_proteins = self.remove_corr_feature(X_train.loc[:,select_proteins],select_proteins_import_df,biomarker_marked_result_path)
        self.info_record("去掉相关性大的特征","计算特征筛选得到的特征两两之间相关性，以0.9为阈值，当两个特征之间的相关性阈值满足r>0.9时保留重要性大的特征，剔除重要性小的特征。最后得到的特征为："+str(final_select_proteins)+",一共有"+str(len(final_select_proteins))+"个蛋白。")

        # 根据新的测试数据集对生物标志物蛋白再一次筛选
        # 是否导入新的测试数据集
        final_X_test_prepared = None
        y_test = None
        test_data_path = gol.get_value("biomarker_marked_test_data_path")
        ...
        # 模型构建
        self.model_build_fuc(final_X_train_prepared, y_train, final_X_test_prepared,y_test,final_X_valid_prepared,y_valid, biomarker_marked_result_path)
    ...
    def remove_corr_feature(self,X_prepared_df,feature_importance_df,biomarker_marked_result_path):
        # 去掉相关性大的特征
        corr_df = X_prepared_df.corr()
        export_data(corr_df, biomarker_marked_result_path, biomarker_feature_select_dirname,
    ...
    def model_build_fuc(self,final_X_train_prepared, y_train, final_X_test_prepared,y_test,final_X_valid_prepared,y_valid, biomarker_marked_result_path):
        scaler = StandardScaler().fit(final_X_train_prepared)
        final_X_train_prepared = pd.DataFrame(scaler.transform(final_X_train_prepared),
                                              columns=final_X_train_prepared.columns,
                                              index=final_X_train_prepared.index)
        export_data(final_X_train_prepared.T, biomarker_marked_result_path, biomarker_data_split_dirname,
                    "train_set_scaler.xlsx", "train set")
        if final_X_test_prepared is not None:
            final_X_test_prepared = pd.DataFrame(StandardScaler().fit_transform(final_X_test_prepared),
                                                 columns=final_X_test_prepared.columns,
                                                 index=final_X_test_prepared.index)
            export_data(final_X_test_prepared.T, biomarker_marked_result_path, biomarker_data_split_dirname,
                        "test_set_scaler.xlsx", "test set")
        if final_X_valid_prepared is not None:
            final_X_valid_prepared = pd.DataFrame(StandardScaler().fit_transform(final_X_valid_prepared),
                                                  columns=final_X_valid_prepared.columns,
                                                  index=final_X_valid_prepared.index)
            export_data(final_X_valid_prepared.T, biomarker_marked_result_path, biomarker_data_split_dirname,
                        "valid_set_scaler.xlsx", "valid set")
        classifier_index = gol.get_value("biomarker_marked_classifier_index")
        classifier_str = gol.get_value("biomarker_marked_classifier")
        ...
        for model_name, model_fuc in classifier_model_dict.items():
            train_result_SGD, valid_result_SGD,model = model_fuc(final_X_train_prepared, y_train, final_X_valid_prepared,
                                                          y_valid, biomarker_marked_result_path)
        ...
        for best_model_name in best_model_name_list:
            test_result =model_in_test(best_model_name, model_dict[best_model_name], final_X_test_prepared, y_test, biomarker_marked_result_path, dirname=biomarker_model_dirname)
